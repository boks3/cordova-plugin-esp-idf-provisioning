...
class ESPIdfProvisioning : CDVPlugin, ESPDeviceConnectionDelegate {
    ...
    var currentEspDevice: ESPDevice? = nil
    ...
    @objc func searchESPDevices (_ command: CDVInvokedUrlCommand) {
        ...
        ESPProvisionManager.shared.searchESPDevices(devicePrefix: deviceNamePrefix, transport: .ble, security: .secure) { bleDevices, error in
            ...
            } else if let bleDevices = bleDevices {
    ...
    @objc func connectBLEDevice (_ command: CDVInvokedUrlCommand){
        ...
        if let espDevice = self.espDevices[deviceName] {
            espDevice.connect(delegate: self) { status in
                switch status {
                    case .connected:
                        self.currentEspDevice = espDevice
                        let pluginResult = CDVPluginResult(status: CDVCommandStatus_OK, messageAs: [
                            "name": espDevice.name,
                            "primaryServiceUuid": "abc"
                        ])
            ...
            let pluginResult = CDVPluginResult(status: CDVCommandStatus_ERROR, messageAs: "Cannot connect device not previously scanned")
            self.commandDelegate!.send(pluginResult, callbackId:command.callbackId)
        }
    }
    ...
    @objc func disconnectBLEDevice (_ command: CDVInvokedUrlCommand){
        let deviceName = command.arguments[0] as! String
        if let currentEspDevice = currentEspDevice {
            if currentEspDevice.name != deviceName {
                let pluginResult = CDVPluginResult(status: CDVCommandStatus_ERROR, messageAs: "Device name not matching current connected ESP device")
                self.commandDelegate!.send(pluginResult, callbackId:command.callbackId)
            } else {
                currentEspDevice.disconnect()
                self.currentEspDevice = nil
                let pluginResult = CDVPluginResult(status: CDVCommandStatus_OK)
                self.commandDelegate!.send(pluginResult, callbackId:command.callbackId)
            }
        } else {
            let pluginResult = CDVPluginResult(status: CDVCommandStatus_ERROR, messageAs: "No device currently connected")
            self.commandDelegate!.send(pluginResult, callbackId:command.callbackId)
        }
    }
    ...
    @objc func scanNetworks (_ command: CDVInvokedUrlCommand) {
        ...
        if let currentEspDevice = currentEspDevice {
            if currentEspDevice.name != deviceName {
                ...
                currentEspDevice.scanWifiList { wifiList, _ in
                    if let list = wifiList {
                        ...
                        let pluginResult = CDVPluginResult(status: CDVCommandStatus_OK, messageAs: [String]())
                        self.commandDelegate!.send(pluginResult, callbackId:command.callbackId)
                    }
                }
            }
    ...
    @objc func provision (_ command: CDVInvokedUrlCommand) {
        ...
        if let currentEspDevice = currentEspDevice {
            if currentEspDevice.name != deviceName {
                ...
                currentEspDevice.provision(ssid: ssid, passPhrase: passphrase) { status in
